...
const PropertyDetails = ({ property, onClose }) => {
  ...
  useEffect(() => {
    const loadLease = async () => {
      ...
      try {
        setLeaseAddress(await contract.methods.leases(property.tokenId).call());
  ...
  const handleApply = async () => {
    ...
    try {
      await contract.methods.applyToRent(intendedStartDate).send({ from: account });
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 sm:p-6" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="relative h-72 w-full bg-gray-100">
          ...
          <div className="absolute bottom-4 left-4 bg-blue-600 text-white font-bold px-3 py-1 rounded-md shadow-md text-lg">
            {property.price} USDC <span className="text-sm font-normal text-blue-100">/month</span>
          </div>
        ...
        <div className="border-b border-gray-200">
          <nav className="flex -mb-px">
            <button
              className={`px-4 py-3 font-medium text-sm border-b-2 ${activeTab === 'details' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
              onClick={() => setActiveTab('details')}
            >
              Property Details
            </button>
            <button
              className={`px-4 py-3 font-medium text-sm border-b-2 ${activeTab === 'amenities' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
              onClick={() => setActiveTab('amenities')}
            >
              Amenities
            </button>
            <button
              className={`px-4 py-3 font-medium text-sm border-b-2 ${activeTab === 'lease' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
        ...
        <div className="flex-1 overflow-auto p-4">
            ...
            <div className="space-y-4">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center bg-gray-100 rounded-lg px-2 py-1">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-amber-500 mr-1">
                    <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clipRule="evenodd" />
                  </svg>
                  <span className="font-medium">{property.rating}</span>
                  <span className="text-gray-500 ml-1">({property.reviewsCount} reviews)</span>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div>
                  <h4 className="font-medium text-gray-800 mb-2">Property Overview</h4>
                  <div className="grid grid-cols-2 gap-y-2">
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <div className="font-medium">{property.bedInfo.split('·')[0].trim()}</div>
                      </div>
                    </div>
                    <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2 text-blue-500">
                      ...
                      <div>
                        ...
                        <div className="font-medium">{property.price} USDC/month</div>
                      </div>
                    </div>
                    <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2 text-blue-500">
                      ...
                      <div>
                        ...
                        <div className="font-medium">{property.available}</div>
                      </div>
                    </div>
                    <div className="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2 text-blue-500">
                      ...
                      <div>
                        ...
                        <div className="font-medium">{property.lease}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <h4 className="font-medium text-gray-800 mb-2">Property Description</h4>
                  <p className="text-gray-600">
                    {property.description || "This beautiful property offers modern living in a fantastic location. Perfect for those looking for comfort and convenience."}
                  </p>
                  
                  {/* Property Manager */}
                  <div className="mt-4">
                    <h4 className="font-medium text-gray-800 mb-2">Listed By</h4>
                    <div className="flex items-center">
          ...
          {activeTab === 'lease' && (
            <div className="space-y-4">
              <div>
                <h4 className="font-medium text-gray-800 mb-3">Lease Information</h4>
                <div className="bg-gray-50 rounded-md p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-500">Lease Term</p>
                      <p className="font-medium">{property.lease}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Available From</p>
                      <p className="font-medium">{property.available}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Monthly Rent</p>
                      <p className="font-medium">{property.price} USDC</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">Security Deposit</p>
                      <p className="font-medium">{property.price * (property.depositInMonths || 2)} USDC ({property.depositInMonths || 2} months)</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className="font-medium text-gray-800 mb-3">NFT Property Information</h4>
                <div className="bg-purple-50 rounded-md p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-500">Lease Contract Address</p>
                      <div className="flex items-center">
                        <p className="font-mono font-medium truncate text-sm">  
                          <a
                            href={`https://${chainId === '0x1' ? '' : 'sepolia.'}arbiscan.io/address/${leaseAddress}`}
                            target="_blank"
                        ...
                        <button className="ml-2 text-blue-600 hover:text-blue-800" title="Copy to clipboard">
        ...
        <div className="border-t border-gray-200 p-4">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
            <div className="flex items-center">
              <span className="text-xl font-bold text-gray-900">{property.price} USDC</span>
              <span className="text-gray-600 ml-1">/month</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex flex-col">
                <label htmlFor="start-date" className="pl-2 text-xs text-gray-500 mb-1">Move-in Date</label>
                <input 
                  type="date" 
                  id="start-date" 
